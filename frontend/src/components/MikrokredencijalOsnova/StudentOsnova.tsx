...
function StudentOsnova({ id, tip, mk, onApliciraj }: Props) {
  ...
  return (
    <div className={styles.card}>
      ...
      <p>
        <strong>Ishodi:</strong> {mk.ishodiUcenja}
      </p>
      {/* <p><strong>Izdato:</strong> {new Date(mk.datumIzdavanja).toLocaleDateString()}</p> */}
      <p>
        <strong>ESPB:</strong> {mk.ESPB}
      </p>
      <p>
        <strong>Nivo:</strong> {mk.nivo}
      </p>
      <p>
        <strong>Trajanje:</strong> {mk.trajanje}
      </p>
      <p>
        <strong>Oblik:</strong> {mk.odrzavanje}
      </p>
      {/* <p>
        <strong>Participacija:</strong> {mk.formaParticipacije}
      </p> */}
      {/* <p><strong>Vrsta provere:</strong> {mk.vrstaProcene}</p> */}
      {/* <p>
        <strong>Supervizija:</strong> {mk.supervizijaProcene}
      </p> */}
      {/* <p><strong>Osiguranje kvaliteta:</strong> {mk.tipOsiguranjaKvaliteta}</p> */}
      {/* <p><strong>Integracija:</strong> {mk.opcijeIntegracije}</p> */}
      <p>
        <strong>Dodatno:</strong> {mk.dodatneInformacije}
      </p>

      {tip === 'informalni' && (
        <button className={styles.button} onClick={() => apliciranje(mk.id)}>
          Apliciraj
        </button>
      )}
    </div>
